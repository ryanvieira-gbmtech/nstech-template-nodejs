...
O padrão Unit Of Work consiste em agrupar multiplas operações de banco de dados em um único lugar. O objetivo principal é garantir a transação entre todos os repositórios de forma uniforme.
Todo repository novo deve extender a função Repository que se localiza no caminho "./interfaces/index.ts".

Para ter acesso aos métodos do repository alvo, basta passar da seguinte forma: 
```typescript
...
export class UserService {
  ...
  async findById(id: UsersId) {
    const user = await this.unitOfWork.getRepository(UserRepository).findById(id);

    return user;
  }

  async createUserWithRole(userData: CreateUserData, roleName: string) {
    await this.unitOfWork.transaction(async (uow) => {
      const userRepository = uow.getRepository(UserRepository);
      const roleRepository = uow.getRepository(RoleRepository);
      ...
      const role = await roleRepository.findByName({ name: roleName });
